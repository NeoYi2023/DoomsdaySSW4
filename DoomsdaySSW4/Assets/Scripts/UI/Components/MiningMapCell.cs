...
/// <summary>
/// 挖矿地图单元格标记组件：
/// - 仅用于标记该格子在挖矿地图逻辑网格中的坐标（网格尺寸由 MiningManager.LAYER_WIDTH/HEIGHT 决定，默认 9x9）
...
{
    [Header("地图坐标（0~10）")]
    [Tooltip("Inspector 允许配置 0~10，用于预留扩展；实际有效范围由 MiningManager.LAYER_WIDTH/HEIGHT 决定（默认 9×9 时为 0~8）。")]
    [Range(0, 10)]
    public int x;
